...
const TILE_SIZE = MAP_DEFAULTS.tileSize;

// Derived endpoint constants
const AUMENTUM_MAPSERVER = SOLANO_MAPSERVER.aumentum;
const AERIAL_TILES = SOLANO_AERIAL_TILES;
const COUNTY_BOUNDARY_URL = `${SOLANO_AGOL_BASE}/County_Boundary/FeatureServer/1`;
const CITY_BOUNDARY_URL = SOLANO_SERVICES.cityBoundary;
...
function toWebMercator(lon: number, lat: number): { x: number; y: number } {
  const x = lon * WEB_MERCATOR_HALF_CIRCUMFERENCE / 180;
  let y = Math.log(Math.tan((90 + lat) * Math.PI / 360)) / (Math.PI / 180);
  y = y * WEB_MERCATOR_HALF_CIRCUMFERENCE / 180;
  return { x, y };
}
