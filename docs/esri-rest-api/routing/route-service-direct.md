...
> Source: [/rest/services-reference/enterprise/routing/route-service-direct/](https://developers.arcgis.com/rest/services-reference/enterprise/routing/route-service-direct/)
...
```
GET https://{{machineName}}/{{serverWebAdaptorName}}/rest/services/{{folderName}}/NetworkAnalysis/NAServer/Route/solve
```
...
```
GET https://{{machineName}}/{{serverWebAdaptorName}}/rest/services/{{serviceName}}/NAServer/{{layerName}}/solve
```
...
| directionsOutputType |  | string | esriDOTStandard | The content and length of the driving directions. Used ifreturnDirections=true. |
| directionsStyleName |  | string | NADesktop | The formatting style for the directions. Used ifreturnDirections=true. |
...
-   TimeWindowStart[datetime](../routing-data-types#datetime)nullable
...
-   TimeWindowEnd[datetime](../routing-data-types#datetime)nullable
...
[`datetime`](../routing-data-types/#datetime)`now`optional
...
-   `startTime` specified as `now`—If the `startTime` parameter is specified as `now`, [live traffic](/rest/services-reference/enterprise/routing/traffic-in-analysis/#live-traffic) will be used where available.
...
-   `startTime` specified as specific day/time— If the `startTime` parameter is specified as a time and day of the week, or live traffic is not available, [historical traffic](/rest/services-reference/enterprise/routing/traffic-in-analysis/#historical-traffic) will be used.
...
-   DepartTime[datetime](../routing-data-types#datetime)nullable
...
-   ArriveTimeUTC[datetime](../routing-data-types#datetime)nullable
    
    The date and time value indicating the arrival time in coordinated universal time (UTC)
    

-   DepartTimeUTC[datetime](../routing-data-types#datetime)nullable
...
-   LocationTypeint enumdefault:0
    ...
    The stop type. The values for this field are copied from the `LocationType` field on the input [stops](#stops).
...
-   CurbApproachint enumdefault:0
    ...
    The direction a vehicle may arrive at and depart from the stop. The values for this field are copied from the `CurbApproach` field on the input [stops](#stops).
...
-   Attr\_\[Cost\]number (non-negative)default:0
    ...
    This field specifies how much additional time spent or distance traveled will occur at the stop. The values for this field are copied from the `Attr_[Cost]` field on the input [stops](#stops).
    
-   Cumul\_\[Cost\]number (non-negative)default:0
...
-   Wait\_\[Cost\]number (non-negative)nullable
...
-   CumulWait\_\[Cost\]number (non-negative)nullable
...
-   Violation\_\[Cost\]number (non-negative)nullable
...
-   CumulViolation\_\[Cost\]number (non-negative)nullable
...
-   TotalWait\_\[Cost\]number (non-negative)nullable
...
-   TotalViolation\_\[Cost\]number (non-negative)nullable
...
```
POST https://{{machineName}}/{{serverWebAdaptorName}}/rest/services/{{folderName}}/NetworkAnalysis/NAServer/Route/solve HTTP/1.1
